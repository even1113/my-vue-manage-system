...
import { fetchWhiteUser, sealWhiteUser, searchWhite } from '../api/user'
...
// 获取表格数据
const getData = () => {
  fetchWhiteUser().then((res) => {
    console.log('获取白名单的res', res, res.data.data.length)
...
// 查询操作
const handleSearch = () => {
  searchWhite(query.name).then((res) => {
    tableData.value = res.data.data
  })
}
// 分页导航
...
const handleEdit = (index: number, row: any) => {
  sealWhiteUser(toRaw(row).id)
  ElMessage.success(`封号用户 ${toRaw(row).id} 成功`)
}
</script>
